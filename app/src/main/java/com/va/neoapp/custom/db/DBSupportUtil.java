...
import com.va.neoapp.custom.db.util.ContentHelper;
...
public abstract class DBSupportUtil {
    ...
    private void checkTablesNAdd(ArrayList<TableDetails> allTableDetails) {
        for (TableDetails tableDetails : allTableDetails) {
    ...
    private static void setTableData(SQLiteDatabase db) {
        for (TableDetails tableDetails : allTableDefinitions) {
            alterRCreateTable(db, tableDetails);
        }
    }

    private static void alterRCreateTable(SQLiteDatabase db, TableDetails tableDetails) {
        // If Table exists
        HashMap<String, String> allFieldsnTypes = tableDetails.getTblFieldsNTypes();
        Set<Map.Entry<String, String>> set = allFieldsnTypes.entrySet();
        if (isTableExists(db, tableDetails.getTableName())) {
            for (Map.Entry<String, String> entry : set) {
                try {
                    db.execSQL("ALTER TABLE " + tableDetails.getTableName() + " ADD COLUMN " + entry.getKey() + " " + entry.getValue() + ";");
                } catch (Exception e) {
            ...
            createtable.append("CREATE TABLE IF NOT EXISTS " + tableDetails.getTableName() + "(_id INTEGER PRIMARY KEY AUTOINCREMENT");
    ...
    public int insertOrUpdateTable(Object object, DBAction insertFlag, @Nullable String where) {
        return insertOrUpdateTable(object.getClass().getSimpleName(), object, insertFlag, where);
    }

    /*public int insertOrUpdateTable(Object object, DBAction insertFlag, @Nullable String where, HashMap<String, String> inputs) {
        ...
        for (String col : inputs.keySet()) {
            try {
                ...
                //e.printStackTrace();
                Log.e("inst_upd_tab_error",e.getMessage());
            }
        }
        return insertOrUpdateTable(object.getClass().getSimpleName(), object, insertFlag, where);
    }*/
     ...
     * @param where
     */
    public <T> void deleteRow(Class<T> classType, String where) {
        deleteRow(classType, where, null);
    }

    public <T> int deleteRow(String talbleName, String where) {
        return deleteRow(talbleName, where, null);
    }
    ...
    private <T> int deleteRow(Class<T> classType, String where, String[] whereArgs) {
        return writableDB.delete(classType.getSimpleName(), where, whereArgs);
    }

    private <T> int deleteRow(String talbeName, String where, String[] whereArgs) {
        return writableDB.delete(talbeName, where, whereArgs);
    }
     ...
     * @return
     */
    private int insertOrUpdateTable(String TableName, Object object, DBAction insertFlag, @Nullable String where) {
    ...
    public <T> T getValuesFromTable(String TableName, String where, Class<T> classType, String orderBy) {
        ArrayList<T> responce = getAllValuesFromTable(TableName, where, classType, orderBy);
        if (responce != null && responce.size() > 0)
            return responce.get(0);
        return null;
    }

    private <T> T getValuesFromTable(String where, Class<T> classType, String orderBy) {
    ...
    private <T> long getCountFromTable(Class<T> classType, String where) {
    ...
    private <T> ArrayList<T> getAllValuesFromTable(String TableName,
    ...
    private QueryBuilders districtBuilder = null;
    private QueryBuilders columnsByBuilder = null;
    private QueryBuilders selectionArgsByBuilder = null;
    private QueryBuilders isHavingBuilder = null;
    private QueryBuilders limitBuilder = null;
    private QueryBuilders groupByBuilder = null;
    private QueryBuilders customQueryBuilder = null;
    ...
    public static class QueryBuilders {
        ...
        QueryBuilders setColumns(String[] columns) {
        ...
        QueryBuilders setSelectionArgs(String[] selectionArgs) {
        ...
        QueryBuilders setHavingString(String havingString) {
            this.havingString = havingString;
            return this;
        }

        QueryBuilders setLimitString(String limitString) {
            this.limitString = limitString;
            return this;
        }

        QueryBuilders setGroupByString(String groupBy) {
         ...
         * @return
         */
        QueryBuilders setCustomQueryBuilder(String customQuery) {
    ...
    private synchronized <T> ArrayList<T> getAllValuesFromTable(String TableName,
        ...
        try {
            ...
            if (customQueryBuilder != null && customQueryBuilder.getCustomQuery() != null)
                ...
                cursor = this.readableDB.query((this.districtBuilder != null), TableName,
                ...
                do {
                    ...
                    for (Field field : clazz.getDeclaredFields()) {
                        try {
                            ...
                            if (!hasInArrayMethod(cursor.getColumnNames(), field.getName()))
                                continue;
...
//                            Log.e("", "");
    ...
    private boolean hasInArrayMethod(String[] values, String value) {
        for (String s : values) {
            if (s.equals(value))
                return true;
        }
